                                                           ...
                                                           Prefetch('client',
                                                                    queryset=Client.objects.all().select_related(
                                                                        'user').only('company_name',
                                                                                     'user__email'))).annotate(
        price=F('service__full_price') * (1 - (F('plan__discount_percent') / 100.00)))

    serializer_class = SubscriptionSerializer

    def list(self, request, *args, **kwargs):  # will change in future
        queryset = self.filter_queryset(self.get_queryset())
        response = super().list(request, *args, **kwargs)
        response_data = {'result': response.data, 'total_amount': queryset.aggregate(total=Sum('price')).get('total')}
